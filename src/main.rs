...
fn main() -> ! {
    ...
    let dc = io.pins.gpio0.into_push_pull_output();
    // Define the reset pin as digital outputs and make it high
    let mut rst = io.pins.gpio3.into_push_pull_output();
    rst.set_high().unwrap();

    // Define the SPI pins and create the SPI interface
    let sck = io.pins.gpio10;
    ...
    // let mut display = Builder::ili943_rgb565(di)
    //     .init(&mut delay, Some(rst))
    //     .unwrap();
